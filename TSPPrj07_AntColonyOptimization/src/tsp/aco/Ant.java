...
public class Ant implements Callable<Ant> {
	...
	private void adjustPheromoneLevel(int x, int y, double distance) {
		boolean flag = false;
		while (!flag) {
			double currentPheromoneLevel = aco.getPheromoneLevelsMatrix()[x][y].doubleValue();
			double updatedPheromoneLevel = (1-RHO)*currentPheromoneLevel + Q/distance;
			if (updatedPheromoneLevel < 0.00) flag = aco.getPheromoneLevelsMatrix()[x][y].compareAndSet(0);
			else flag = aco.getPheromoneLevelsMatrix()[x][y].compareAndSet(updatedPheromoneLevel);
		}
	}
	...
	private double getTPNumerator(int x, int y) {
		...
		double phermoneLevel = aco.getPheromoneLevelsMatrix()[y][x].doubleValue();
